...
def MaxSAT(formula, assignments):
    ...
    for values in product([True, False], repeat=len(all_vars)):
        current_assignment = dict(zip(all_vars, values))
        satisfiable_count = 0

        # Count satisfiable clauses for the current assignment
        for clause in formula:
            if any((lit > 0 and current_assignment[abs(lit)]) or (lit < 0 and not current_assignment[abs(lit)]) for lit in clause):
                satisfiable_count += 1

        # Update the best assignment if this one is better
        if satisfiable_count > max_satisfiable:
            max_satisfiable = satisfiable_count
            best_assignment = current_assignment

    # Print the best assignment and the number of satisfiable clauses
    print("Best Assignment:", best_assignment)
    print(f"Maximum number of satisfiable clauses: {max_satisfiable}")

def CDCL(formula):
    d=formula #copy of the formula
    assignments = {}
    trail = []
    implication_graph = {}
    decision_level = 0
